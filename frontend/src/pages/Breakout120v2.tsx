...
const Breakout120v2 = () => {
    ...
    const { data: results, isLoading, error } = useQuery({
        queryKey: ['breakout120v2', searchTrigger],
        queryFn: async () => {
            // Remove hyphen for API (YYYY-MM -> YYYYMM)
            const ymd = searchMonth.replace('-', '')
            const response = await axios.get('/api/analysis/breakout120v2', {
    ...
    const memoMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['breakout120v2'] })
    ...
    return (
        <div className="max-w-full mx-auto p-4 flex flex-col h-full">
            ...
            <h2 className="text-xl font-bold mb-4 text-gray-800">분석 &gt; 120일 돌파2</h2>
